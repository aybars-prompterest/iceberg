...
import { Layout, Megaphone, CreditCard, Users, Sparkles, Handshake, Bot, MousePointerClick } from 'lucide-react'
  ...
  return (
    <div className="p-6 lg:p-8 flex flex-col gap-6 w-full">
      ...
      <div className="flex flex-col gap-6">
        {activeTab === 'hero' && (
          <>
            <SectionHeader title="Hero Bölümü" description="Ana sayfanın en üstündeki hero alanı." />
        ...
        {activeTab === 'announcement' && (
          <>
            <SectionHeader title="Duyuru Bandı" description="Hero altında görünen duyuru çubuğu." />
        ...
        {activeTab === 'services' && (
          <>
            <SectionHeader title="Hizmet Kartları" description="Ana sayfadaki 3 hizmet kartı." />

            {[0, 1, 2].map(i => (
              <div key={i} className="bg-bg-surface border border-border rounded-xl p-5 flex flex-col gap-4">
                <p className={subHeadingClass}>Kart {i + 1}</p>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <Field label="Başlık" value={values[`services_card_${i}_title`] ?? ''} onChange={v => set(`services_card_${i}_title`, v)} />
                  <Field label="Link Metni" value={values[`services_card_${i}_link_text`] ?? ''} onChange={v => set(`services_card_${i}_link_text`, v)} />
                </div>
                <Field
                  label="Açıklama"
                  ...
                  rows={2}
                />
                <Field label="Bağlantı (href)" value={values[`services_card_${i}_href`] ?? ''} onChange={v => set(`services_card_${i}_href`, v)} />
              </div>
            ))}
          </>
        )}
        ...
        {activeTab === 'cta' && (
          <>
            <SectionHeader title="CTA Banner" description="Sayfadaki aksiyon çağrısı banner'ı." />
        ...
        {activeTab === 'testimonials' && (
          <>
            <SectionHeader title="Müşteri Yorumları" description="Ana sayfadaki 4 müşteri yorumu kartı." />

            {[0, 1, 2, 3].map(i => (
              <div key={i} className="bg-bg-surface border border-border rounded-xl p-5 flex flex-col gap-4">
                <p className={subHeadingClass}>Yorum {i + 1}</p>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <Field label="İsim" value={values[`testimonial_${i}_name`] ?? ''} onChange={v => set(`testimonial_${i}_name`, v)} />
                  <Field label="Ünvan" value={values[`testimonial_${i}_title`] ?? ''} onChange={v => set(`testimonial_${i}_title`, v)} />
                </div>
                <Field
                  label="Yorum"
        ...
        {activeTab === 'features' && (
          <>
            <SectionHeader title="Özellikler" description="Ana sayfadaki 6 özellik kartı." />
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
              {[0, 1, 2, 3, 4, 5].map(i => (
                <div key={i} className="bg-bg-surface border border-border rounded-xl p-5 flex flex-col gap-4">
                  ...
                  <Field label="Başlık" value={values[`feature_${i}_title`] ?? ''} onChange={v => set(`feature_${i}_title`, v)} />
                  <Field
                    label="Açıklama"
        ...
        {activeTab === 'partners' && (
          <>
            <SectionHeader title="Partnerler" description="Ana sayfadaki partner/marka logoları." />

            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
              {[0, 1, 2, 3, 4].map(i => (
                <div key={i} className="bg-bg-surface border border-border rounded-xl p-5 flex flex-col gap-3">
                  <p className={subHeadingClass}>Partner {i + 1}</p>
                  <Field label="Adı" value={values[`partner_${i}_name`] ?? ''} onChange={v => set(`partner_${i}_name`, v)} />
                  <Field
                    label="Logo URL"
        ...
        {activeTab === 'bot' && (
          <>
            <SectionHeader title="Blog Bot Ayarları" description="Otomatik blog yazısı üreten bot'un yapılandırması." />
